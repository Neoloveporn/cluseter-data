...
Rock_use_name = ['CS','tensile_strength',
...
df_des.to_excel('G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据统计信息.xlsx', sheet_name='Sheet1')
print(Property_trans.info())

print(Property_trans[Property_trans.isnull().values ==True])

Property_trans.to_excel('G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据转换后结果.xlsx', sheet_name='Sheet1')
...
attributes = ["Top","CS", 'tensile_strength','deformation_modulus','elastic_modulus',"poisson_ratio",
                 "soften_coefficient"] 
              
scatter_matrix(Pro_prepared[attributes], figsize=(20,15))
save_fig("2meach_scatter_matrix_plot")


import seaborn as sns

scatter_use_name = ['CS','tensile_strength',
                 'deformation_modulus','elastic_modulus',"poisson_ratio",
...
#sns.set(rc={"axes.facecolor":"white", "xtick.direction":"in", "ytick.direction":"in",
            #"xtick.major.size":4, "ytick.major.size":4, "grid.linewidth":0.5})
...
save_fig("2meach_inertia_vs_k_plot")
plt.show()
...
save_fig("2meach_silhouette_score_vs_k_plot")
plt.show()
...
save_fig("2meach_silhouette_analysis_plot")
plt.show()
...
print(center) 
center1 = (center*std)+mean 
print(round(center1,2))

Rock_property["classfication"]=pd.DataFrame(y_pred2)
Rock_property.to_excel("G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据分类结果.xlsx")
center1.to_excel("G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据分类的中心向量.xlsx")                 
...
data=[go.Parcoords(
        ...
        dimensions = list([
            ...
            dict(range = [420,10000],
                label = 'deformation_modulus', values = df.iloc[:,10],
                tickvals = [round(x,2) for x in [420 + i * 958 for i in range(11)]]),
            
            dict(range = [400,10000],
                label = 'elastic_modulus', values = df.iloc[:,11],
                tickvals = [round(x,2) for x in [400 + i * 960 for i in range(11)]]),
            
            dict(range = [0.5,0],
                label = 'poisson_ratio', values = df.iloc[:,12],
                tickvals = [round(x,2) for x in [0.5 - i * 0.05 for i in range(11)]]),
...
py.offline.plot(fig, filename = '力学属性数据原始数据.html'
               )
...
print(df1.columns)
df1.to_excel("G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据特征缩放分类结果.xlsx")
data=[go.Parcoords(
        ...
        dimensions = list([
            ...
            dict(range = [-2,4], constraintrange = [-2,4], 
                label = 'CS', values = df1.iloc[:,0]),
            dict(range = [-2,3],
                label = 'tensile_strength', values = df1.iloc[:,1]),
            dict(range = [-2,5],
                label = 'deformation_modulus', values = df1.iloc[:,2]),
            dict(range = [-2,5.5],
                label = 'elastic_modulus', values = df1.iloc[:,3]),
            dict(range = [-3,3],
                label = 'poisson_ratio', values = df1.iloc[:,4]),
            dict(range = [-2,3],
                label = 'soften_coefficient', values = df1.iloc[:,5]),
            dict(range = [-1,5],
                label = 'classfication', values = df1['classfication']),            
...
py.offline.plot(data, filename = '力学属性数据特征缩放数据.html'
               )
...
df2.to_excel("G:\工作项目资料文件夹\中英文论文写作\广安小论文\广安小论文3\\力学属性数据分类结果岩性数量.xlsx")
